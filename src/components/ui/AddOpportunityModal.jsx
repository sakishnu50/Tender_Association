...
export default function AddOpportunityModal({ isOpen, onClose, onAdd }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (
      ...
      setErrorMsg(
        'Please fill in all required fields before adding the opportunity.'
      );
      return;
    }
    ...
    const newOpportunity = {
      ...
      scoreBreakdown: [
        {
          label: 'Sector Match',
          score: Math.min(10, calculatedScore + 0.2),
          max: 10,
        },
        {
          label: 'Country/Market Match',
          score: calculatedScore,
          max: 10,
        },
        {
          label: 'Past Experience',
          score: Math.max(7.0, calculatedScore - 0.3),
          max: 10,
        },
        {
      ...
      aiAnalysis: {
        ...
        strengths: [
          `Strong alignment with ${formData.sector.trim()}`,
          `Strategic geographic fit in ${formData.location.trim()}`,
        ],
        risks: ['Review bid timeline and documentation requirements'],
        recommendation:
          calculatedScore >= 8.5
            ? 'Highly Recommended – Pursue'
